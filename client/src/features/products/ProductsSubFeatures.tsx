...
export const ProductsSubFeatures = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const fromApi: CategoryEntry[] = (data.data || []).map((c: any) => ({
        _id: c._id,
        name: c.name,
        vatRate: c.vatRate ?? 0,
        vatType: c.vatType ?? 'exclusive',
        loyaltyPoints: c.loyaltyPoints ?? 0,
      }));
      // Merge any legacy localStorage entries not yet in the API
      const legacy = (() => {
        try {
      ...
      const toMigrate = legacy.filter(c => !apiNames.has(c.name));
      for (const cat of toMigrate) {
        try { await api.post('/categories', cat); } catch { /* skip duplicates */ }
      }
      if (toMigrate.length > 0) {
        localStorage.removeItem('customProductCategories');
        const { data: refreshed } = await api.get('/categories');
        setCustomCategories((refreshed.data || []).map((c: any) => ({
          _id: c._id, name: c.name, vatRate: c.vatRate ?? 0,
          vatType: c.vatType ?? 'exclusive', loyaltyPoints: c.loyaltyPoints ?? 0,
        })));
      } else {
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    if (customCategories.some(c => c.name === newCatName.trim())) {
      notifications.show({ title: 'Error', message: 'Category already exists', color: 'red', icon: <IconX size={16} /> });
      return;
    }
    const newEntry = {
    ...
    try {
      await api.post('/categories', newEntry);
      ...
      notifications.show({
        title: 'Success',
        message: `Category "${newEntry.name}" added`,
        color: 'green',
        icon: <IconCheck size={16} />,
      });
    } catch (err: any) {
                              ...
                              <Table.Tbody>
                                {displayedProducts.map((p) => (
                                  <Table.Tr key={p._id}>
                                    ...
                                    <Table.Td style={{ textAlign: 'right' }}>{p.discount > 0 ? (<><span style={{ textDecoration: 'line-through', color: 'gray' }}>{formatMoney(p.price)}</span> <span style={{ color: 'red', fontWeight: 600 }}>{formatMoney(p.displayedPrice)} ({p.discount}%)</span></>) : (<>{formatMoney(p.price)}</>)}</Table.Td>
                                  </Table.Tr>
                                ))}
                                {products.length === 0 && (
                                  <Table.Tr>
                  ...
                  <Table striped highlightOnHover>
                    <Table.Thead>
                      <Table.Tr>
                        ...
                        <Table.Th style={{ width: 160 }}>Online Price ({currencySymbol()})</Table.Th>
              ...
              <Table striped highlightOnHover>
                <Table.Thead style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1 }}>
                  <Table.Tr>
                    <Table.Th>Product Name</Table.Th>
                    <Table.Th>SKU</Table.Th>
                    <Table.Th style={{ width: 140 }}>Cost Price ({currencySymbol()})</Table.Th>
                    <Table.Th style={{ width: 140 }}>Selling Price ({currencySymbol()})</Table.Th>
                ...
                <Table.Tbody>
                  {uploadedRows.map((row, idx) => (
                    <Table.Tr key={idx}>
                      ...
                      <Table.Td style={{ textAlign: 'right' }}>{formatMoney(row.costPrice)}</Table.Td>
                      <Table.Td style={{ textAlign: 'right' }}>{formatMoney(row.price)}</Table.Td>
