...
export const loadQuickProducts = (): QuickProductButton[] => {
  // In Electron, always return defaults synchronously — fetchQuickProducts() loads from SQLite async
  if (window.electronAPI) return DEFAULT_QUICK_PRODUCTS;
  try {
...
const saveQuickProducts = (items: QuickProductButton[]) => {
  // In Electron, SQLite is the source of truth — skip localStorage
...
export const fetchQuickProducts = async (): Promise<QuickProductButton[]> => {
  try {
    if (window.electronAPI) {
      // Read directly from SQLite via IPC
      const items = await window.electronAPI.settings.getQuickProducts();
      return Array.isArray(items) && items.length > 0 ? items : DEFAULT_QUICK_PRODUCTS;
    }
...
const persistQuickProducts = async (items: QuickProductButton[]) => {
  saveQuickProducts(items);
  if (window.electronAPI) {
    // Write directly to SQLite via IPC
    await window.electronAPI.settings.updateQuickProducts(items);
  } else {
